...
function SubitemsAnimados({ visivel, children }: { visivel: boolean; children: React.ReactNode }) {
  ...
  // Quando mede a altura real, dispara a animação
  useEffect(() => {
  ...
  // Antes de medir, renderiza fora da tela para capturar altura
  if (!medido) {
    return (
      <View
...
const ls = StyleSheet.create({
  ...
  catContainer: {
    backgroundColor: C.fumo, borderRadius: 12, marginBottom: 8,
    borderWidth: 1, borderColor: '#3d2510', overflow: 'hidden',
  },
  ...
  countBadge: {
    backgroundColor: C.brasa + '33', borderRadius: 20,
  ...
  subItem: {
    ...
    borderBottomWidth: 1, borderBottomColor: '#2d1a0a', gap: 10,
  },
  checkbox: {
    width: 22, height: 22, borderRadius: 6, borderWidth: 2,
    borderColor: '#5a3010', alignItems: 'center', justifyContent: 'center',
  },
  ...
  tagBtn: {
    backgroundColor: '#3d2510', borderRadius: 6,
...
export default function CardapioScreen({ convidados, onVoltar, onCalcular }: Props) {
  ...
  const handleCalcular = () => {
    const algumSelecionado = categorias.some(c => c.ativo && c.subitens.some(s => s.selecionado));
    if (!algumSelecionado) {
    ...
    <View style={S.flex1}>
      ...
      <View style={S.container}>
                                ...
                                <TouchableOpacity
                                  ...
                                  onPress={() => {
                                    setModalQtd({
                                      ...
                                      label: sub.label, qtd: sub.manual ?? 0,
        ...
        <View style={S.bottomBar}>
          <TouchableOpacity
            style={[S.btnPrimary, { marginHorizontal: 0 }]}
        ...
        <View style={S.modalOverlay}>
          <View style={S.modalBox}>
            ...
            <Text style={S.modalSub}>
              Total em kg — automático: {modalQtd?.auto} kg
            </Text>
            <TextInput
              style={S.modalInput}
              value={tempQtd}
            ...
            <View style={S.modalBtns}>
              <TouchableOpacity
                ...
                onPress={() => {
                  setCategorias(prev => prev.map(c =>
                    c.id === modalQtd?.catId
                      ? { ...c, subitens: c.subitens.map(s => s.id === modalQtd?.subId ? { ...s, manual: null } : s) }
                      : c
                  ));
                  setModalQtd(null);
                }}
