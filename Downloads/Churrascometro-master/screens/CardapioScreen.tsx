...
import {
  ...
  StatusBar, Alert, Switch, Modal, StyleSheet, Animated,
} from 'react-native';
...
// ─── Tipos ──────────────────────────────────────────────────────────────────
type SubItem = {
  id: string; label: string; preco: number;
  qtdPorPessoa: number; selecionado: boolean; manual?: number | null;
};

type CategoriaComSub = {
  id: string; emoji: string; label: string;
  ativo: boolean; expandido: boolean; subitens: SubItem[];
};

// ─── Dados padrão ────────────────────────────────────────────────────────────
const CATEGORIAS_DEFAULT: CategoriaComSub[] = [
  {
...
function ChevronAnimado({ expandido }: { expandido: boolean }) {
  ...
  useEffect(() => {
    Animated.timing(rotacao, { toValue: expandido ? 1 : 0, duration: 280, useNativeDriver: true }).start();
  }, [expandido]);
  const rotate = rotacao.interpolate({ inputRange: [0, 1], outputRange: ['0deg', '180deg'] });
  return (
    <Animated.View style={{ transform: [{ rotate }] }}>
      <Ionicons name="chevron-down" size={20} color={C.brasa2} />
    </Animated.View>
  );
}

// ── Subitens animados com altura real ─────────────────────────────────────────
function SubitemsAnimados({ visivel, children }: { visivel: boolean; children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!medido || alturaReal === 0) return;
    if (visivel) {
      Animated.parallel([
        Animated.timing(alturaAnim,    { toValue: alturaReal, duration: 300, useNativeDriver: false }),
        Animated.timing(opacidadeAnim, { toValue: 1,          duration: 250, useNativeDriver: false }),
      ]).start();
    } else {
      Animated.parallel([
        Animated.timing(alturaAnim,    { toValue: 0, duration: 250, useNativeDriver: false }),
        Animated.timing(opacidadeAnim, { toValue: 0, duration: 200, useNativeDriver: false }),
      ]).start();
    }
  }, [visivel, medido, alturaReal]);
    ...
    return (
      <View
        ...
        onLayout={e => {
          ...
          if (h > 0) {
            setAlturaReal(h); setMedido(true);
            alturaAnim.setValue(visivel ? h : 0);
            opacidadeAnim.setValue(visivel ? 1 : 0);
          }
        }}
      >
        {children}
  ...
  return (
    <Animated.View style={{ height: alturaAnim, opacity: opacidadeAnim, overflow: 'hidden' }}>
      {children}
    </Animated.View>
  );
}
...
const ls = StyleSheet.create({
  sectionLabel: { color: C.brasa2, fontSize: 11, fontWeight: '800', letterSpacing: 2, marginTop: 20, marginBottom: 8 },
  catContainer: { backgroundColor: C.fumo, borderRadius: 12, marginBottom: 8, borderWidth: 1, borderColor: '#3d2510', overflow: 'hidden' },
  ...
  countBadge:   { backgroundColor: C.brasa + '33', borderRadius: 20, paddingHorizontal: 8, paddingVertical: 2, alignSelf: 'flex-start' },
  countBadgeText: { color: C.brasa, fontSize: 11, fontWeight: '700' },
  expandBtn:    { padding: 4 },
  subitemsContainer: { borderTopWidth: 1, borderTopColor: '#2d1a0a' },
  subItem:      { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 14, paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: '#2d1a0a', gap: 10 },
  checkbox:     { width: 22, height: 22, borderRadius: 6, borderWidth: 2, borderColor: '#5a3010', alignItems: 'center', justifyContent: 'center' },
  checkboxChecked: { backgroundColor: C.brasa, borderColor: C.brasa },
  subLabelBtn:  { flex: 1 },
  ...
  tagBtn:       { backgroundColor: '#3d2510', borderRadius: 6, paddingHorizontal: 7, paddingVertical: 4 },
...
type Props = {
  ...
  categoriasIniciais: CategoriaComSub[];
  cardapioSimplesInicial: ItemCardapio[];
  onVoltar: () => void;
...
export default function CardapioScreen({
  convidados, categoriasIniciais, cardapioSimplesInicial, onVoltar, onCalcular,
}: Props) {
  // Inicializa com estado preservado (se existir) ou com o padrão
  const [categorias, setCategorias] = useState<CategoriaComSub[]>(
    categoriasIniciais.length > 0 ? categoriasIniciais : CATEGORIAS_DEFAULT
  );
  const [cardapio, setCardapio] = useState<ItemCardapio[]>(cardapioSimplesInicial);
  ...
  const toggleSubitem = (catId: string, subId: string) =>
    setCategorias(prev => prev.map(c =>
      c.id === catId
        ? { ...c, subitens: c.subitens.map(s => s.id === subId ? { ...s, selecionado: !s.selecionado } : s) }
        : c
    ));
  ...
  const salvarPrecoSub = () => {
    ...
    setCategorias(prev => prev.map(c =>
      c.id === modalPreco.catId
        ? { ...c, subitens: c.subitens.map(s => s.id === modalPreco.subId ? { ...s, preco: val } : s) }
        : c
    ));
    setModalPreco(null);
  };
  ...
  const salvarQtdSub = () => {
    ...
    setCategorias(prev => prev.map(c =>
      c.id === modalQtd.catId
        ? { ...c, subitens: c.subitens.map(s => s.id === modalQtd.subId ? { ...s, manual: val } : s) }
        : c
    ));
    setModalQtd(null);
  };
  ...
  const handleCalcular = () => {
    const algum = categorias.some(c => c.ativo && c.subitens.some(s => s.selecionado));
    if (!algum) { Alert.alert('Atenção', 'Selecione pelo menos um tipo de carne, frango ou linguiça!'); return; }
    onCalcular(cardapio, categorias);
  };

  const contarSelecionados = (cat: CategoriaComSub) => cat.subitens.filter(s => s.selecionado).length;
    ...
    <View style={S.flex1}>
        ...
        <ScrollView contentContainerStyle={{ padding: 14, paddingBottom: 120 }} keyboardShouldPersistTaps="handled">
            ...
            return (
              <View key={cat.id} style={ls.catContainer}>
                <View style={ls.catHeader}>
                  <Switch value={cat.ativo} onValueChange={() => toggleCategoria(cat.id)}
                    trackColor={{ false: C.fumo2, true: C.brasa + '88' }}
                    thumbColor={cat.ativo ? C.brasa : '#555'} />
                  <Text style={ls.catEmoji}>{cat.emoji}</Text>
                  <TouchableOpacity style={ls.catLabelBtn} onPress={() => cat.ativo && toggleExpandir(cat.id)} activeOpacity={0.7}>
                    <Text style={[ls.catLabel, !cat.ativo && { color: '#5a3010' }]}>{cat.label}</Text>
                    {selecionados > 0 && (
                  ...
                  {cat.ativo && (
                    <TouchableOpacity onPress={() => toggleExpandir(cat.id)} style={ls.expandBtn}>
                      <ChevronAnimado expandido={cat.expandido} />
                    </TouchableOpacity>
                  )}
                </View>

                {cat.ativo && (
                  <SubitemsAnimados visivel={cat.expandido}>
                    <View style={ls.subitemsContainer}>
                      {cat.subitens.map((sub, idx) => {
                        const autoQtd = ((sub.qtdPorPessoa * convidados.length) / 1000).toFixed(1);
                        return (
                          <View key={sub.id} style={[ls.subItem, idx === cat.subitens.length - 1 && { borderBottomWidth: 0 }]}>
                            <TouchableOpacity style={[ls.checkbox, sub.selecionado && ls.checkboxChecked]} onPress={() => toggleSubitem(cat.id, sub.id)}>
                              {sub.selecionado && <Ionicons name="checkmark" size={14} color="#fff" />}
                            </TouchableOpacity>
                            <TouchableOpacity style={ls.subLabelBtn} onPress={() => toggleSubitem(cat.id, sub.id)}>
                              <Text style={[ls.subLabel, sub.selecionado && { color: C.branco }]}>{sub.label}</Text>
                              <Text style={ls.subQtdAuto}>{sub.manual != null ? `${sub.manual} kg` : `~${autoQtd} kg`}</Text>
                            </TouchableOpacity>
                            {sub.selecionado && (
                              <View style={ls.subAcoes}>
                                <TouchableOpacity style={ls.tagBtn} onPress={() => {
                                  setModalQtd({ catId: cat.id, subId: sub.id, label: sub.label, qtd: sub.manual ?? 0, auto: parseFloat(autoQtd) });
                                  setTempQtd(sub.manual != null ? String(sub.manual) : '');
                                }}>
                                  <Text style={ls.tagBtnText}>{sub.manual != null ? `📏 ${sub.manual}kg` : '📏 auto'}</Text>
                                </TouchableOpacity>
                                <TouchableOpacity style={ls.tagBtn} onPress={() => {
          ...
          {cardapio.map(item => (
            <View key={item.id} style={[S.cardapioRow, !item.ativo && S.cardapioRowOff]}>
              <Switch value={item.ativo} onValueChange={() => toggleItem(item.id)}
                trackColor={{ false: C.fumo2, true: C.brasa + '88' }}
              ...
              {item.ativo && (
                <TouchableOpacity style={S.tagBtn} onPress={() => { setModalPrecoSimples(item); setTempPreco(String(item.preco)); }}>
      ...
      <Modal visible={!!modalPreco} transparent animationType="fade">
        <View style={S.modalOverlay}><View style={S.modalBox}>
          ...
          <TextInput style={S.modalInput} value={tempPreco} onChangeText={setTempPreco} keyboardType="numeric" placeholder="Ex: 80.00" placeholderTextColor="#8a6a50" autoFocus />
          <View style={S.modalBtns}>
            <TouchableOpacity style={S.modalBtnCancel} onPress={() => setModalPreco(null)}><Text style={S.modalBtnCancelText}>Cancelar</Text></TouchableOpacity>
            <TouchableOpacity style={S.modalBtnOk} onPress={salvarPrecoSub}><Text style={S.modalBtnOkText}>Salvar</Text></TouchableOpacity>
          </View>
        </View></View>
      </Modal>
      ...
      <Modal visible={!!modalQtd} transparent animationType="fade">
        <View style={S.modalOverlay}><View style={S.modalBox}>
          ...
          <TextInput style={S.modalInput} value={tempQtd} onChangeText={setTempQtd} keyboardType="numeric" placeholder="Ex: 2.5" placeholderTextColor="#8a6a50" autoFocus />
          <View style={S.modalBtns}>
            <TouchableOpacity style={S.modalBtnCancel} onPress={() => {
              setCategorias(prev => prev.map(c => c.id === modalQtd?.catId
                ? { ...c, subitens: c.subitens.map(s => s.id === modalQtd?.subId ? { ...s, manual: null } : s) } : c));
              setModalQtd(null);
            }}><Text style={S.modalBtnCancelText}>Resetar</Text></TouchableOpacity>
            <TouchableOpacity style={S.modalBtnOk} onPress={salvarQtdSub}><Text style={S.modalBtnOkText}>Salvar</Text></TouchableOpacity>
          </View>
        </View></View>
      </Modal>
      ...
      <Modal visible={!!modalPrecoSimples} transparent animationType="fade">
        <View style={S.modalOverlay}><View style={S.modalBox}>
          ...
          <TextInput style={S.modalInput} value={tempPreco} onChangeText={setTempPreco} keyboardType="numeric" placeholder="Ex: 8.00" placeholderTextColor="#8a6a50" autoFocus />
          <View style={S.modalBtns}>
            <TouchableOpacity style={S.modalBtnCancel} onPress={() => setModalPrecoSimples(null)}><Text style={S.modalBtnCancelText}>Cancelar</Text></TouchableOpacity>
            <TouchableOpacity style={S.modalBtnOk} onPress={salvarPrecoSimples}><Text style={S.modalBtnOkText}>Salvar</Text></TouchableOpacity>
          </View>
        </View></View>
      </Modal>
    </View>
